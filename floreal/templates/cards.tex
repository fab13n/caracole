...
{# more than 16 lines, 1 per column, 180mm #}
{# up to 16 lines, 2 per column, 85mm #}
{# up to x lines, 3 per column, 58mm #}
\begin{adjustbox}{minipage=[c][\orderheight]{13cm}}
\begin{tabular}[c]{|r|l|rl|r|p{1cm}|p{1cm}|}
\cline{3-7}
\multicolumn{2}{c|}{\bf {{ur.user.first_name}} {{ur.user.last_name}}}&
\multicolumn{3}{c|}{Commandés}&
\multicolumn{2}{c|}{Livrés}\\
\hline
{% for pc in ur.orders.purchases %}{% if pc.ordered %}
{{pc.product.name}} &
{{pc.product.price|price}}/{{pc.product.unit}} &
{{pc.ordered|qty}} &
{{pc.product.unit}} &
{{pc.price|price}} & & \\
{% endif %}{%endfor %}
\hline \hline
\multicolumn{1}{|r}{\hspace{40mm}Total} && {{ur.weight}} & kg & {\bf {{ur.price|price}}} &&\\
\hline
\end{tabular}

{\scriptsize\em Circuit court {{t.subgroup.network.name}},
 sous-groupe {{t.subgroup.name}},
 commande {{d.delivery.name}} }
\end{adjustbox}
{% if d.max_order_size > 16 %}
